...
func parseModel(model interface{}) *metadata {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if defaultSort != nil {
			metadata.DefaultSorts = []*Sort{defaultSort}
		}
	}
	...
	if methodValue.IsValid() {
		...
		if len(values) == 1 {
			if sorts, ok := values[0].Interface().([]*Sort); ok && len(sorts) > 0 {
...
// If the field is sortable and is also specified as the default sort, return a Sort model as well
...
//	  Field5 string
//	}
func parseSortable(field reflect.StructField) (*Sort, bool) {
